...
ftxui::Component MultiOptionSelector( std::vector<std::pair<std::string,bool*>> const& options, std::string const& label = "" );

//! Styled and labeled window with vertical radio buttons
ftxui::Component RadioSelector( std::vector<std::string> const* entries, int * selected, std::string const& label = "" );
...
ftxui::Component SpinBox( int & value, std::string const& title = "" );


//! Button that executes a background task. Displays success/error messages returned by the task and shows loading animation  
ftxui::Component WorkerButton( ftxui::ScreenInteractive & screen, std::function<std::string()> task, std::string const& label = "" );

//! Text input component with autocompletion integrated (using tabs)
ftxui::Component FileInput( ftxui::StringRef content, ftxui::StringRef placeholder, ftxui::InputOption options = {} );
